"""
Risk-Constrained Agent - 保守防御型策略
...
MODEL_OUT_PATH = os.path.join(MODEL_OUT_DIR, "risk_constrained_agent.pth")
...
print(f"🚀 Risk-Constrained Agent Training - DEFENSIVE")
...
class RiskConstrainedEnv:
    ...
    def __init__(self, tickers: List[str], price_df: pd.DataFrame, model_predict,
                 ...
                 transaction_cost: float = 0.0008, risk_free_rate: float = 0.02,
                 dd_lambda: float = 20.0):  # 高回撤惩罚
    ...
    def step(self, action: np.ndarray) -> StepResult:
        ...
        # 🛡️ 保守策略：强制高现金比例
        if weights[-1] < 0.30:  # 现金不低于30%
            weights[-1] = 0.30
            stock_weights = weights[:self.n]
            if stock_weights.sum() > 0:
                weights[:self.n] = stock_weights / stock_weights.sum() * 0.70

        # 🛡️ 极度分散：单股不超过20%
        for i in range(self.n):
            if weights[i] > 0.20:
                excess = weights[i] - 0.20
                weights[i] = 0.20
                weights[-1] += excess  # 超出部分转为现金
        ...
        if turnover_ratio < 0.01:
            ...
            reward += 0.15
        elif turnover_ratio > 0.03:  # 惩罚过高换手
            reward -= 0.3
        
        # 6. ✅ 鼓励高现金比例 (30-50%)
        cash_ratio = self.cash / new_value if new_value > 0 else 1.0
        if 0.30 <= cash_ratio <= 0.50:
            reward += 0.4
        elif cash_ratio >= 0.40:
            reward += 0.6  # 更高现金有额外奖励
        ...
        if new_value > 0:
            ...
            if max_position_ratio < 0.22:  # 分散度奖励
                reward += 0.3
            elif max_position_ratio > 0.30:  # 过度集中惩罚
                reward -= 0.3
        
        # 8. 交易成本惩罚 (2x)
        reward -= cost / (prev_value + 1e-8) * 2.0
...
class RiskConstrainedNet(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, hidden: int = 256):
        ...
        self.shared = nn.Sequential(
            ...
            nn.LayerNorm(hidden),
            nn.ReLU(),
            nn.Dropout(0.15),  # 更高dropout增加保守性
...
class RiskConstrainedAgent:
    def __init__(self, state_dim, action_dim, lr=2e-4, gamma=0.99, lam=0.95,
                 clip_eps=0.2, ent_coef=0.01, vf_coef=0.5, device='cpu'):
        self.ac = RiskConstrainedNet(state_dim, action_dim).to(device)
...
def train():
    ...
    agent = RiskConstrainedAgent(state_dim=14, action_dim=4, lr=2e-4, gamma=0.99, lam=0.95,
                                 clip_eps=0.2, ent_coef=0.01, vf_coef=0.5, device=DEVICE)

    num_episodes = 2000
    steps_per_update = 200

    print(f"\n🎯 Starting DEFENSIVE training for {num_episodes} episodes...")
    print("=" * 60)
    ...
    for ep in range(1, num_episodes + 1):
        ...
        env = RiskConstrainedEnv(tickers=tickers, price_df=price_df, model_predict=model_pred,
        ...
        while True:
            ...
            ep_drawdowns.append(step_res.info.get('drawdown', 0))
        ...
        all_drawdowns.append(np.max(ep_drawdowns))
        ...
        if ep % 50 == 0:
            ...
            avg_dd = np.mean(all_drawdowns[-100:]) if len(all_drawdowns) >= 100 else np.mean(all_drawdowns)
            ...
            print(
                ...
                f"Turn: {avg_turnover*100:5.1f}% | Cash: {avg_cash*100:4.1f}% | MaxDD: {avg_dd*100:4.1f}%"
            )
    ...
    try:
        ...
        plt.title('Risk-Constrained: Training Returns')
        plt.legend()
        ...
        plt.title('Final Portfolio Values (CONSERVATIVE)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.subplot(2, 3, 4)
        plt.plot([d*100 for d in all_drawdowns], alpha=0.3, label='Max Drawdown', color='red')
        if len(all_drawdowns) >= 100:
            ma = pd.Series([d*100 for d in all_drawdowns]).rolling(100).mean()
            plt.plot(ma, label='100-ep MA', linewidth=2, color='darkred')
        plt.axhline(y=6, color='orange', linestyle='--', label='Target <6%', alpha=0.7)
        plt.xlabel('Episode')
        plt.ylabel('Max Drawdown (%)')
        plt.title('Risk Control (Lower is Better)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        if len(all_cash_ratios) >= 100:
            ...
            plt.plot(ma, label='100-ep MA', linewidth=2, color='red')
        plt.axhline(y=30, color='blue', linestyle='--', label='Min 30%', alpha=0.7)
        plt.axhline(y=40, color='green', linestyle='--', label='Target 40%', alpha=0.7)
        plt.xlabel('Episode')
        plt.ylabel('Cash Ratio (%)')
        plt.title('Conservative Allocation (HIGH CASH)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        if len(all_turnovers) >= 100:
            ...
            plt.plot(ma, label='100-ep MA', linewidth=2, color='red')
        plt.axhline(y=2, color='green', linestyle='--', label='Target <2%', alpha=0.7)
        plt.xlabel('Episode')
        plt.ylabel('Turnover Rate (%)')
        plt.title('Trading Activity (LOW)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plot_path = os.path.join(MODEL_OUT_DIR, "risk_constrained_training_curves.png")
    ...
    print("\n" + "=" * 60)
    print("🎉 Risk-Constrained Defensive Training Complete!")
    print("=" * 60)
    ...
    final_dd = np.mean(all_drawdowns[-100:]) if len(all_drawdowns) >= 100 else np.mean(all_drawdowns)
    ...
    print(f"  Average max drawdown: {final_dd*100:.1f}%")
    print("=" * 60)
